 ...
 */
export async function generateTwitterPost(insights: any, tone: string) {
...
`.replace('${hook}', hook)
 .replace('${insights.main_topics.join(', ')}', insights.main_topics.join(', '))
 .replace('${toneInstruction}', toneInstruction)
 .replace('${BANNED_PHRASES.join(', ')}', BANNED_PHRASES.join(', '));
 ...
 */
export async function generateTwitterThread(insights: any, tone: string) {
  const toneInstruction = TONE_INSTRUCTIONS[tone] || TONE_INSTRUCTIONS.casual;
...
`.replace('${insights.curiosity_hooks.join(' | ')}', insights.curiosity_hooks.join(' | '))
 .replace('${insights.lessons.join(' | ')}', insights.lessons.join(' | '))
 .replace('${insights.quotes.join(' | ')}', insights.quotes.join(' | '))
 .replace('${insights.surprising_facts.join(' | ')}', insights.surprising_facts.join(' | '))
 .replace('${toneInstruction}', toneInstruction)
 .replace('${BANNED_PHRASES.join(', ')}', BANNED_PHRASES.join(', '));
 ...
 */
export async function generateLinkedInPost(insights: any, tone: string) {
...
`.replace('${pickOne(insights.stories) || insights.interesting_moments[0]}', pickOne(insights.stories) || insights.interesting_moments[0] || '')
 .replace('${insights.lessons.join(' | ')}', insights.lessons.join(' | '))
 .replace('${insights.actionable_advice.join(' | ')}', insights.actionable_advice.join(' | '))
 .replace('${toneInstruction}', toneInstruction)
 .replace('${BANNED_PHRASES.join(', ')}', BANNED_PHRASES.join(', '));
 ...
 */
export async function generateInstagramCaption(insights: any, tone: string) {
  const toneInstruction = TONE_INSTRUCTIONS[tone] || TONE_INSTRUCTIONS.casual;
...
`.replace('${pickOne(insights.curiosity_hooks) || pickOne(insights.surprising_facts)}', pickOne(insights.curiosity_hooks) || pickOne(insights.surprising_facts) || '')
 .replace('${toneInstruction}', toneInstruction)
 .replace('${BANNED_PHRASES.join(', ')}', BANNED_PHRASES.join(', '));
 ...
 */
export async function generateInstagramCarousel(insights: any, tone: string) {
  const toneInstruction = TONE_INSTRUCTIONS[tone] || TONE_INSTRUCTIONS.casual;
 ...
 */
export async function generateFacebookPost(insights: any, tone: string) {
  const toneInstruction = TONE_INSTRUCTIONS[tone] || TONE_INSTRUCTIONS.casual;
 ...
 */
export async function generateYouTubeCommunityPost(insights: any, tone: string) {
  const toneInstruction = TONE_INSTRUCTIONS[tone] || TONE_INSTRUCTIONS.casual;
 ...
 */
export async function generateReelScript(insights: any, tone: string) {
  const toneInstruction = TONE_INSTRUCTIONS[tone] || TONE_INSTRUCTIONS.casual;
